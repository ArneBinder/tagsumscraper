...
def prepare_for_html(content, as_list=False):
    if as_list:
...
def intents_split_to_dynamicContent(intents_split, answers_all, nbr_posts, max_queries, only_query_nbr=None):
    ...
    for i, intent_id in enumerate(intents_split):
        ...
        for post_pos in range(nbr_posts):
            ...
            if post_pos < len(current_answers_split_sorted):
                ...
                answer_splits = prepare_for_html(re.sub(r'\s*\(\d+\)\s*$', '', current_answers_split_sorted[post_pos][1]), as_list=True)

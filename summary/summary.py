    ...
    for i, intent_id in enumerate(intents_split):
        ...
        for post_pos in range(nbr_posts):
            ...
            if post_pos < len(current_answers_split_sorted):
                ...
                answer_splits = prepare_for_html(re.sub(r'\s*\(\d+\)\s*$', '', current_answers_split_sorted[post_pos][1]), format_as=format_as)
...
def main(base_path: ("Path to the base directory", 'option', 'p')='scrape_10',
         ...
         format_as: ("How to format the sentence entries", 'option', 'f', str, [FORMAT_LIST, FORMAT_CHECKBOXES])=FORMAT_LIST
         ):
